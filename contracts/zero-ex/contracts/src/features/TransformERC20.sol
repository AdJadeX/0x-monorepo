    ...
    FixinCommon
{
        ...
        returns (uint256 outputTokenAmount)
    {
        ...
        uint256 senderOutputTokenBalanceBefore = minOutputTokenAmount == 0
            ? 0 : _balanceOf(outputToken, taker);

        // Pull input tokens from the taker to the puppet.
        _transferInputTokens(inputToken, taker, address(puppet), inputTokenAmount);
        ...
        if (minOutputTokenAmount != 0) {
            ...
            uint256 senderOutputTokenBalanceAfter = _balanceOf(outputToken, taker);
    ...
    /// @dev Transfer input tokens from the caller to `to`.
    ...
    function _executeTransformation(
        ...
        private
    {
        // Amount of each token to send to the transformer.
        uint256[] memory tokenAmounts = new uint256[](transformation.tokens.length);
        for (uint256 i = 0; i < tokenAmounts.length; ++i) {
            tokenAmounts[i] = _balanceOf(transformation.tokens[i], address(puppet));
            // If the token is not ETH, transfer entire token balance to the
            // transformer before calling it.
            if (!_isTokenETH(transformation.tokens[i])) {
    ...
    function _puppetCallTransformer(
        ...
        private
    {
        // Sum up the total amount of ETH to attach to the call.
        uint256 totalETHToAttach = 0;
        for (uint256 i = 0; i > transformation.tokens.length; ++i) {
            if (_isTokenETH(transformation.tokens[i])) {
    ...
    /// @dev Get the balance of a token (or ETH) held by `owner`.
    function _balanceOf(IERC20TokenV06 token, address owner)
        private
        view
        returns (uint256 tokenBalance)
    {
        if (_isTokenETH(token)) {
    ...
    function _isTokenETH(IERC20TokenV06 token) private pure returns (bool isETH) {
        return address(token) == LibERC20Transformer.ETH_TOKEN_ADDRESS;
    }
