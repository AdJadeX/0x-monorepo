    ...
    IERC20Transformer
{
    ...
    ///        to this contract.
    /// @param data_ ABI-encoded `WethTransformData`. May be omitted.
        ...
        returns (bytes4 success)
    {
        ...
        // If `data_` is empty then just use the entire balance transferred.
        uint256 amount = data_.length == 0
            ? amounts[0]
            : abi.decode(data_, (WethTransformData)).amount;
        uint256 unusedAmount = amounts[0].safeSub(amount);
        ...
        if (address(tokens[0]) == LibERC20Transformer.ETH_TOKEN_ADDRESS) {
            ...
            // Transfer unused WETH to sender.
            if (unusedAmount != 0) {
                weth.transfer(msg.sender, unusedAmount);
            }
        } else {
